...
Set Parameters: Enter values for $n, T, V$, and the Van der Waals constants $a$ and $b$ (default values for Oxygen are pre-filled).

Calculate: Click the "Run Simulation & Plot" button.
...
Explore Deviations: Adjust the temperature ($T$) or constants ($a$ and $b$) to see how these factors shift the real gas behavior.
